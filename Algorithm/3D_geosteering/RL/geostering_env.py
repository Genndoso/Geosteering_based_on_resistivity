...
class Geosteering_env(Env):
    def __init__(self, action_space: np.array, prod_map: np.array, start_point=[0, 20, 20], init_azimut = 0, init_inclination = 0,
    ...
    def _action_reward(self, action):
        ...
        if len(self.incl_l) < 3:
            ...
            self.az_arr.append(self.azi_l[-1] + self.items[action][1])
        else:
            incl2 = self.incl_l[-1]
            azi2 = self.azi_l[-1]
            incl1 = self.incl_l[-2]
            ...
            if dls_val >= self.angle_constraint * self.length:
                k = 1
        if (self.state[0] >= self.prod_map.shape[0] - 30) or (self.state[0] <= 10) or \
                (self.state[1] >= self.prod_map.shape[1] - 30) or (self.state[1] <= 10) or \
                (self.state[2] >= self.prod_map.shape[2] - 30) or (self.state[2] <= 10):
            k = 1
            self.done = True
        if k == 1:
            next_incl_diff = 0
        ...
        OFV = interpn(self.points, self.map_3d, self.state, method = 'nearest')/np.linalg.norm(vec_diff) - dls_val * 10
    ...
    #         """
    #         Compute fitness function based on current productivity potential, length of trajectory, angle constraints
    ...
    def step(self, action):
        ...
        return np.array(self.state), rw, self.done, info
